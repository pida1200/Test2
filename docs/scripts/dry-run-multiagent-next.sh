...
_run() {
  ...
  case "$artifact:$gate:$vk" in
    ...
    multiagent/bug:gate/pending:*)     echo "→ Vývojář · composer-2.5-fast (bug mimo pipeline)" ;;
...
resolve_artifact() {
  local -a labels=("$@") a l
  for a in multiagent/verdikt multiagent/analyza multiagent/implementace multiagent/testy multiagent/pipeline multiagent/bug; do
