...
npm run check:ma       # pipeline-sync, regex, wiki, next-lib, merge-lib, …
...
- Komentář `<!-- multiagent-next -->` — další role + model (`auto`) + `/m #N`
- Sekce `<!-- multiagent:prehled -->` v `[PIPELINE]` — tabulka fází
- Komentář `<!-- multiagent-gate-check -->` — chyby formátu verdiktu
- Po MERGE-PENDING: issue `[MERGE] … Ano / Ne?` (`multiagent/merge-review`)
- Po ≥3 NO-GO: `gate/blocked` na pipeline

## Wiki UI

Po syncu ověř Home + sidebar: [Test2 wiki](https://github.com/pida1200/Test2/wiki).  
Drift seed ↔ UI = `sync-wiki-to-github.sh` nebo push na `main` (wiki-sync).

## Learning log

Po větším MA běhu: `docs/learning-log.md` (Integrátor při handoff / close).
